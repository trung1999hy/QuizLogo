package com.bmt_team.logoquiz.inapp;
...
public class PurchaseInAppActivity extends AppCompatActivity implements PurchaseInAppAdapter.OnClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        imvBack = findViewById(R.id.btn_back);
        listData = findViewById(R.id.rcv_purchase);
        initViews();
...
// lắng nghe trả về trạng thái mua
        billingClient.acknowledgePurchase(acknowledgePurchaseParams, billingResult -> {
            if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                App.getInstance().getPreference().setPremium(1);
    ...
    private void setupResult(String proId, int quantity) {
        ...
        int totalCoin = App.getInstance().getPreference().getValueCoin();
        int remainCoin = totalCoin + getCoinFromKey(proId) * quantity;
        App.getInstance().getPreference().setValueCoin(remainCoin);
